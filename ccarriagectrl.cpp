...
CCarriageCtrl::CCarriageCtrl()
{
    ...
    m_Warning4_status= false;
     m_Warning1_status1= false;
     m_Warning2_status1= false;
     m_Warning3_status1= false;
     m_Warning4_status1= false;
...
void CCarriageCtrl::SetPassengerWarningColor( __in int nCarID,int num,bool warning2,QColor color)
{
    ...
    else
    {
        if( num==1)
        {
            m_Warning1_status1=warning2;
        }else if(num==2)
        {
            m_Warning2_status1=warning2;
        }
        else if(num==3)
        {
            m_Warning3_status1=warning2;
        }
        else if(num==4)
        {
            m_Warning4_status1=warning2;
        }
    }
...
//绘制乘客报警的
    if(m_Warning1_status==1)
    {
        ...
        m_pPainter->drawRect(QRect(x+1, y+height*4/7-10,width/8,height*2/7-8));
        m_pPainter->setPen(Qt::white);
        m_pPainter->drawText(QRect(x+1, y+height*4/7-10,width/8,height*2/7-8), Qt::AlignCenter, QObject::trUtf8("1"));
    }
    if(m_Warning2_status==1)
    {
        m_pPainter->setBrush(m_PassengerWarningColor);
        m_pPainter->drawRect(QRect(x+width*3/4-1+width/8, y+height*4/7-10,width/8,height*2/7-8));
        m_pPainter->setPen(Qt::white);
        m_pPainter->drawText(QRect(x+width*3/4-1+width/8, y+height*4/7-10,width/8,height*2/7-8), Qt::AlignCenter, QObject::trUtf8("2"));
    }
    if(m_Warning3_status==1)
    {
        m_pPainter->setBrush(m_PassengerWarningColor);
        m_pPainter->drawRect(QRect(x+1, y+height*4/7+3+4,width/8,height*2/7-8));
        m_pPainter->setPen(Qt::white);
        m_pPainter->drawText(QRect(x+1, y+height*4/7+3+4,width/8,height*2/7-8), Qt::AlignCenter, QObject::trUtf8("3"));
    }
    if(m_Warning4_status==1)
    {
        m_pPainter->setBrush(m_PassengerWarningColor);
        m_pPainter->drawRect(QRect(x+width*3/4-1+width/8, y+height*4/7+3+4,width/8,height*2/7-8));
        m_pPainter->setPen(Qt::white);
        m_pPainter->drawText(QRect(x+width*3/4-1+width/8, y+height*4/7+3+4,width/8,height*2/7-8), Qt::AlignCenter, QObject::trUtf8("4"));
    }

    if(m_Warning4_status1==1)
    {
        m_pPainter->setBrush(m_PassengerWarningColor);
        m_pPainter->drawRect(QRect(x+1, y+height*4/7-10,width/8,height*2/7-8));
        m_pPainter->setPen(Qt::white);
        m_pPainter->drawText(QRect(x+1, y+height*4/7-10,width/8,height*2/7-8), Qt::AlignCenter, QObject::trUtf8("4"));
    }
    if(m_Warning3_status1==1)
    {
        m_pPainter->setBrush(m_PassengerWarningColor);
        m_pPainter->drawRect(QRect(x+width*3/4-1+width/8, y+height*4/7-10,width/8,height*2/7-8));
        m_pPainter->setPen(Qt::white);
        m_pPainter->drawText(QRect(x+width*3/4-1+width/8, y+height*4/7-10,width/8,height*2/7-8), Qt::AlignCenter, QObject::trUtf8("3"));
    }
    if(m_Warning2_status1==1)
    {
        m_pPainter->setBrush(m_PassengerWarningColor);
        m_pPainter->drawRect(QRect(x+1, y+height*4/7+3+4,width/8,height*2/7-8));
        m_pPainter->setPen(Qt::white);
        m_pPainter->drawText(QRect(x+1, y+height*4/7+3+4,width/8,height*2/7-8), Qt::AlignCenter, QObject::trUtf8("2"));
    }
    if(m_Warning1_status1==1)
    {
        m_pPainter->setBrush(m_PassengerWarningColor);
        m_pPainter->drawRect(QRect(x+width*3/4-1+width/8, y+height*4/7+3+4,width/8,height*2/7-8));
        m_pPainter->setPen(Qt::white);
        m_pPainter->drawText(QRect(x+width*3/4-1+width/8, y+height*4/7+3+4,width/8,height*2/7-8), Qt::AlignCenter, QObject::trUtf8("1"));
    }
                                       ...
                                       int up, bool isolated )
{
    ...
    painter->setPen( QPen( Qt::yellow, 15, Qt::SolidLine, Qt::RoundCap, Qt::MiterJoin ) );
    if( up==1 )
    {
        QPointF points[] = {
            QPointF( -50, -30 ), // 0
